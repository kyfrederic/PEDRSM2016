...
var path = process.env.PWD + '/.file/ejs/'+nom;
...
var path = process.env.PWD + '/.file/js/'+nom;
buffer = new Buffer(contenu);
...
var path = process.env.PWD + '/.file/css/'+nom;
buffer = new Buffer(contenu);
...
{
	files = fs.readdirSync(process.env.PWD + '/.file/ejs/');
	//console.log(files);
return files;
}
...
{
return fs.readdirSync(process.env.PWD + '/.file/js/');
}

listeFichiersCSS= function()
{
return fs.readdirSync(process.env.PWD + '/.file/css/');
}
...
{
  return fs.readFileSync(process.env.PWD + '/.file/js/'+nom, 'utf8');
}
else if(nom.substring(nom.length-4,nom.length)==".ejs")
     {
            return fs.readFileSync(process.env.PWD + '/.file/ejs/'+nom, 'utf8');
      }
       else if(nom.substring(nom.length-4,nom.length)==".css")
            {
             return fs.readFileSync(process.env.PWD + '/.file/css/'+nom, 'utf8');
             }
}
